// Safety-grade modification experiment (docs/40 follow-up).
//
// Three modification tasks on once_cell where correctness depends on
// knowledge held in EXTERNALIZED comments/docs. Conditions:
//   blind : stripped code, sidecars never mentioned, no tools
//   lens  : stripped code + ./notes,./owners with FETCH-FIRST wording
//           (the docs/40 §6 operational recommendation)
//   full  : original code with comments
//
// Fix rounds only for build errors (max 2 extra). Behavior/contract failures
// are terminal and the feedback never names the violated rule.
//
// usage: node experiments/safety-mods/run.mjs --crate <once_cell checkout>
//        [--models m1,m2] [--conds blind,lens,full] [--ids ...] [--out file]

import { execFileSync, spawnSync } from "node:child_process";
import { writeFileSync, readFileSync, mkdirSync, cpSync, rmSync, existsSync, chmodSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { MODS, scanOrderings } from "./tasks.mjs";

const HERE = dirname(fileURLToPath(import.meta.url));
const ROOT = join(HERE, "../..");
const WORK = process.env.TMPDIR ? join(process.env.TMPDIR, "safety-mods") : "/tmp/claude/safety-mods";
...
// ---- prep: pristine crate (minimal manifest) + stripped view ----
...
Before changing any code you MUST consult \`./notes\` for the module and the items you are about to modify or imitate, and follow every contract documented there.`
...
// ---- graders ----
// returns {gate: "build"|"behavior"|"contract"|null, feedback}
export function grade(mod, code, tag) {
  const work = join(WORK, "mods", tag);
  rmSync(work, { recursive: true, force: true });
  mkdirSync(work, { recursive: true });
  cpSync(CRATE, join(work, "crate"), { recursive: true });
  const produced = mod.inject_tests ? code + "\n" + mod.inject_tests : code;
  writeFileSync(join(work, "crate", "src", `${mod.target}.rs`), produced);
  ...
  if (mod.fixture_main) {
    ...
    if (b.status !== 0) return { gate: "build", feedback: `cargo build failed:\n${(b.stderr ?? "").slice(0, 6000)}` };
    const r = spawnSync(join(WORK, "target-shared", "debug", "fx"), [], { encoding: "utf8", timeout: 60000 });
    if (r.status !== 0 || !(r.stdout ?? "").includes("OK")) {
      return { gate: "behavior", feedback: `behavior test failed (exit ${r.status}):\n${(r.stdout ?? "").slice(0, 800)}\n${(r.stderr ?? "").slice(0, 1500)}` };
    }
  }
  if (mod.inject_tests) {
    const t = spawnSync("cargo", ["test", "--quiet", "--manifest-path", join(work, "crate", "Cargo.toml"), "grader_is_running"], { encoding: "utf8", timeout: 600000, env });
    if (t.status !== 0) {
      const out = (t.stdout ?? "") + (t.stderr ?? "");
      if (/error\[|^error(\[|:)/m.test(out) && !/test result/.test(out)) {
        return { gate: "build", feedback: `cargo test failed to build:\n${out.slice(0, 6000)}` };
      }
      return { gate: "behavior", feedback: `grader tests failed:\n${out.slice(0, 2500)}` };
    }
  }
  if (mod.scanRegion) {
    const violation = scanOrderings(mod.scanRegion(code), mod.scanKind);
    if (violation) {
      return { gate: "contract", feedback: `The change violates a documented contract of this module (${violation}).` };
    }
  }
  return { gate: null, feedback: null };
}
...
if (process.env.SAFETY_MODS_LIB !== "1") {
  for (const model of MODELS) {
    ...
    for (const cond of CONDS) {
      for (const mod of MODS) {
        ...
        let rounds = 0, gate = null, feedback = null, totTok = { in: 0, out: 0 }, totCost = 0, totTurns = 0;
        while (rounds < 3) {
          ...
          const g = grade(mod, code, `${short}-${cond}-${mod.id}-r${rounds}`);
          rounds++;
          gate = g.gate; feedback = g.feedback;
          if (gate === null) break;           // success
          if (gate !== "build") break;        // behavior/contract: terminal
          prompt = `${CTX[cond]}\n\n# Task\n\n${mod.task}\n\nYour previous attempt:\n\`\`\`rust\n${code}\n\`\`\`\n\nIt failed to build:\n${feedback}\n\nOutput the COMPLETE corrected \`src/${mod.target}.rs\` file in a single \`\`\`rust code block.`;
        }
        const rec = {
          type: "mod", model: short, cond, id: mod.id,
          ok: gate === null, gate, rounds, notesCalls: readCalls(dir), turns: totTurns,
          tokens: totTok, cost: totCost, secs: (Date.now() - t0) / 1000,
          feedback: gate === null ? null : feedback?.slice(0, 400),
        };
        results.push(rec);
        save();
        console.log(`[${short}/${cond}] ${mod.id}: ${rec.ok ? `OK r${rounds}` : `FAIL@${gate}`} notes=${rec.notesCalls.length} out=${totTok.out}`);
      }
    }
  }
  save();
